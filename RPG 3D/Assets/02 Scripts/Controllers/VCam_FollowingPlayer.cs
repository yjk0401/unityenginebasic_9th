...
public class VCam_FollowingPlayer : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        _followTarget.Rotate(Vector3.right, mouseY * _rotateSpeedX * Time.deltaTime, Space.Self);
        ...
        if (Mathf.Abs(Input.mouseScrollDelta.y) > _scrollThreshold) 
        {
            _vCam.m_Lens.FieldOfView = 
                Mathf.Clamp(Input.mouseScrollDelta.y * _scrollSpeed * Time.deltaTime,
                            _fovMin,
                            _fovMax);
        }
    }
    ...
    private float ClampAngle(float angle, float min, float max)
    {
        ...
        // min 범위를 음수세팅 할 수 있도록 (-30도 부터 30도 까지 쓰게 제한)
        if (angle > 180.0f) 
        {
            angle -= 360.0f;        
        }
