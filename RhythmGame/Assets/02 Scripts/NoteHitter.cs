...
public class NoteHitter : MonoBehaviour
{
    ...
    private void Hit() 
    {
        ...
        if (cols.Length > 0) 
        {
            Transform closesetNote = 
                cols.OrderBy(x => Mathf.Abs(x.transform.position.y - transform.position.y)).First().transform;

            float distance = Mathf.Abs(closesetNote.transform.position.y - transform.position.y);
            ...
            _hitEffect.transform.position = closesetNote.transform.position;
            _hitEffect.Play();
            Destroy(closesetNote.gameObject);
        }
